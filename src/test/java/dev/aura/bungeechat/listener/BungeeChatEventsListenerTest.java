...
public class BungeeChatEventsListenerTest {
    ...
    public void setupReceiverAndListener() {
        if(mockReceiver != null)
            pluginManager.unregisterListener(mockReceiver);
    ...
    public void doubleJoinInOrderLeaveTest() {
        ...
        ArgumentCaptor<BungeeChatJoinEvent> joinCaptor = ArgumentCaptor.forClass(BungeeChatJoinEvent.class);
        Mockito.verify(mockReceiver).onBungeeChatJoin(joinCaptor.capture());
        assertSame(joinCaptor.getValue().getPlayer(), player1); // join is fired only on the first player

        ArgumentCaptor<BungeeChatLeaveEvent> leaveCaptor = ArgumentCaptor.forClass(BungeeChatLeaveEvent.class);
        Mockito.verify(mockReceiver).onBungeeChatLeave(leaveCaptor.capture());
        assertSame(leaveCaptor.getValue().getPlayer(), player1); // leave is fired only on the first player
