    ...
    public void execute(CommandSender sender, String[] args) {
        if (PermissionManager.hasPermission(sender, Permission.COMMAND_CLEAR_CHAT)) {
            if (args.length == 0) {
                ...
                final int lines = BungeecordModuleManager.CLEAR_CHAT_MODULE.getModuleSection().getInt("emptyLines");
                ...
                if (args[0].equalsIgnoreCase("local")) {
                    ...
                    MessagesService.sendToMatchingPlayers(Message.CLEARED_LOCAL.get(sender),
                            MessagesService.getLocalPredicate(serverName));
                } else if (args[0].equalsIgnoreCase("global")) {
                    clearGlobalChat(lines);

                    MessagesService.sendToMatchingPlayers(Message.CLEARED_GLOBAL.get(sender),
