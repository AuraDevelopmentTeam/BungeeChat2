...
public class TabCompletionListener implements Listener {
  ...
  public void onTabComplete(TabCompleteEvent e) {
    ...
    if (!PermissionManager.hasPermission(
        (CommandSender) e.getSender(), Permission.USE_TAB_COMPLETE)) return;
    ...
    Stream<BungeeChatAccount> stream =
        AccountManager.getAccountsForPartialName(partialPlayerName).stream();

    if (ModuleManager.isModuleActive(BungeecordModuleManager.VANISHER_MODULE)
        && !PermissionManager.hasPermission(
            (CommandSender) e.getSender(), Permission.COMMAND_VANISH_VIEW)) {
